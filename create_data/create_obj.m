function [vList,fList,ratioVector,centroid,rMatrix] = create_obj(objData, shapePath)

centroid = objData.centroid;
...
if abs(dot(objOri,xBasis)) > abs(dot(objOri,yBasis))
    ratioVector = [ratio*maxX,ratio*maxY,ratioZ*maxZ];
else
    ratioVector = [ratio*maxY,ratio*maxX,ratioZ*maxZ];
end
